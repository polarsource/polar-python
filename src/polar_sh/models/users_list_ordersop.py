...
class UsersListOrdersSecurityTypedDict(TypedDict):
    open_id_connect: NotRequired[str]
    http_bearer: NotRequired[str]
    

class UsersListOrdersSecurity(BaseModel):
...
class UsersListOrdersRequestTypedDict(TypedDict):
...
class UsersListOrdersRequest(BaseModel):
    organization_id: Annotated[OptionalNullable[QueryParamOrganizationIDFilter], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Filter by organization ID."""
    product_id: Annotated[OptionalNullable[ProductIDFilter], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Filter by product ID."""
    product_price_type: Annotated[OptionalNullable[ProductPriceTypeFilter], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Filter by product price type. `recurring` will return orders corresponding to subscriptions creations or renewals. `one_time` will return orders corresponding to one-time purchases."""
    subscription_id: Annotated[OptionalNullable[QueryParamSubscriptionIDFilter], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Filter by subscription ID."""
    query: Annotated[OptionalNullable[str], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Search by product or organization name."""
    ...
    r"""Size of a page, defaults to 10. Maximum is 100."""
    sorting: Annotated[OptionalNullable[List[UserOrderSortProperty]], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Sorting criterion. Several criteria can be used simultaneously and will be applied in order. Add a minus sign `-` before the criteria name to sort by descending order."""
    
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["organization_id", "product_id", "product_price_type", "subscription_id", "query", "page", "limit", "sorting"]
        nullable_fields = ["organization_id", "product_id", "product_price_type", "subscription_id", "query", "sorting"]
        null_default_fields = []
...
class UsersListOrdersResponseTypedDict(TypedDict):
    result: ListResourceUserOrderTypedDict
    

class UsersListOrdersResponse(BaseModel):
    next: Callable[[], Optional[UsersListOrdersResponse]]
