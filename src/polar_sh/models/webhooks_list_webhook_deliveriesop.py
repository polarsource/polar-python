...
class WebhooksListWebhookDeliveriesSecurityTypedDict(TypedDict):
    open_id_connect: NotRequired[str]
    http_bearer: NotRequired[str]
    

class WebhooksListWebhookDeliveriesSecurity(BaseModel):
...
class WebhooksListWebhookDeliveriesRequestTypedDict(TypedDict):
    endpoint_id: NotRequired[Nullable[str]]
...
class WebhooksListWebhookDeliveriesRequest(BaseModel):
    endpoint_id: Annotated[OptionalNullable[str], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Filter by webhook endpoint ID."""
