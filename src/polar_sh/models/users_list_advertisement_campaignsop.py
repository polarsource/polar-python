...
class UsersListAdvertisementCampaignsSecurityTypedDict(TypedDict):
    open_id_connect: NotRequired[str]
    http_bearer: NotRequired[str]
    

class UsersListAdvertisementCampaignsSecurity(BaseModel):
...
class UsersListAdvertisementCampaignsRequestTypedDict(TypedDict):
...
class UsersListAdvertisementCampaignsRequest(BaseModel):
    ...
    r"""Size of a page, defaults to 10. Maximum is 100."""
    sorting: Annotated[OptionalNullable[List[UserAdvertisementSortProperty]], FieldMetadata(query=QueryParamMetadata(style="form", explode=True))] = UNSET
    r"""Sorting criterion. Several criteria can be used simultaneously and will be applied in order. Add a minus sign `-` before the criteria name to sort by descending order."""
...
class UsersListAdvertisementCampaignsResponseTypedDict(TypedDict):
    result: ListResourceUserAdvertisementCampaignTypedDict
    

class UsersListAdvertisementCampaignsResponse(BaseModel):
    next: Callable[[], Optional[UsersListAdvertisementCampaignsResponse]]
